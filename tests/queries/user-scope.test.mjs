...
test("transaction scope includes only the current user plus anonymous CSV fallback when Plaid is excluded", () => {
  assert.equal(
    transactionUserScopePredicate(),
...
test("account scope includes current user accounts and anonymous CSV account rows", () => {
  assert.equal(
    accountUserScopePredicate(),
...
test("anonymous CSV dedupe keeps one canonical anonymous CSV row while preserving user rows", () => {
  assert.equal(
    anonymousCsvDedupePredicate(),
