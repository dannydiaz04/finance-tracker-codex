...
export function getSuggestionPlans(planned: PlannedOverride[]) {
  return planned.filter(({ plan }) => plan.ruleSuggestion);
}
...
export function buildSuggestionPersistenceRows(input: {
  ...
  const staleRows = input.pending
    .filter(
      (row) =>
        row.transaction_id &&
        keepByTransaction.has(row.transaction_id) &&
